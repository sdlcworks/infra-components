...
inputs:
  ...
  - name: workload-target-identity
    description: Address-level target meaning supplied by the workload boundary when non-serverless targets are declared - target form and port, health semantics, and the instance-churn cadence needed to tune admission and drain.
...
outputs:
  ...
  - name: target-attachment-point
    description: Attachment-slot identity provided to the workload boundary so it can steer per-instance addresses into the distribution point as instances are created, replaced, and drained.
