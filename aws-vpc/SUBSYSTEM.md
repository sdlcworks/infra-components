...
inputs:
  - name: network-fabric-intent
    description: Environment-declared address, placement, reachability, and egress posture that defines the network fabric.
...
outputs:
  - name: placement-context
    description: Network identity and placement tiers needed by systems that must reside inside the fabric.
  - name: attachment-surface
    description: Network identity, address range, private placement, and routing surfaces needed by an interconnect hub.
